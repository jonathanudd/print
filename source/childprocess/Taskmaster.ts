...
module Print.Childprocess {
	export class Taskmaster {
		...
		constructor(path: string, private token: string, pullRequestNumber: number, user: string, private name: string, private organization: string, branch: string) {
		...
		manage(name: string, allJobsFinishedCallback: (executionResults: ExecutionResult[]) => void) {
			var results: ExecutionResult[] = [];
			if (!fs.existsSync(String(this.folderPath)))
				fs.mkdirSync(String(this.folderPath));
			
			var queue = new JobQueue(name, allJobsFinishedCallback);
			...
			if (!fs.existsSync(this.folderPath + '/' + this.name)) { 
				queue.addJob(new Job("Git clone", "git", ["clone", "-b", this.branch, "--single-branch", userUrl], this.folderPath));
				queue.addJob(new Job("Git pull upstream", "git", ["pull", organizationUrl, "master"], primaryRepositoryFolderPath));
				queue.addJob(new Job("Git reset to first HEAD", "git", ["reset", "--hard", "HEAD~0"], primaryRepositoryFolderPath));
			}
			else {
				queue.addJob(new Job("Git fetch origin", "git", ["fetch", userUrl], primaryRepositoryFolderPath));
				queue.addJob(new Job("Git reset to origin", "git", ["reset", "--hard", "origin/" + this.branch], primaryRepositoryFolderPath));
				queue.addJob(new Job("Git pull upstream", "git", ["pull", organizationUrl, "master"], primaryRepositoryFolderPath));
				queue.addJob(new Job("Git reset to first HEAD", "git", ["reset", "--hard", "HEAD~0"], primaryRepositoryFolderPath));
			}
			...
			if (this.repositoryConfiguration.secondary != "none") {
				if (!fs.existsSync(this.folderPath + '/' + this.repositoryConfiguration.secondary)) { 
					queue.addJob(new Job("Git clone secondary upstream", "git", ["clone", "-b", "master", "--single-branch", secondaryOrganizationUrl], this.folderPath));
				}
				else {
					queue.addJob(new Job("Git fetch secondary origin", "git", ["fetch", secondaryOrganizationUrl], secondaryRepositoryFolderPath));
					queue.addJob(new Job("Git reset to secondary origin", "git", ["reset", "--hard", "HEAD~0"], secondaryRepositoryFolderPath));
				}
			}
			...
			this.actions.forEach(action => {
				...
				queue.addJob(new Job(action.task, action.task, args, path));
			});
